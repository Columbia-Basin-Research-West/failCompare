...
# THESE THROW ERRORS
# fc_fit(taglife,"vitality.4p",SEs=F)


# indivdual model fitting
# sapply(names(fc_mod_ls),function(x){fc_fit(model = x,time = taglife)$mod_objs})
# sapply(names(fc_mod_ls),function(x){fc_fit(model = x,time = taglife,SEs = F)$mod_objs})

# simultaneous model fitting
# fmods_all=fc_fit(taglife,"all",SEs = F)

# PROBLEMATIC CODE RELATED TO THE SES
# 
names(fc_fit(taglife,"weibull",SEs = F)$par_tab)

fc_fit(taglife,"weibull",SEs = T)
fc_fit(taglife,"weibull3",SEs = T)

fc_fit(taglife,c("weibull","weibull3"),SEs = T)
fc_fit(taglife,c("weibull3","weibull"),SEs = T)


fc_fit(taglife,c("weibull","weibull3"),SEs = F)
fc_fit(taglife,c("weibull3","weibull"),SEs = F)


m1=fc_fit(taglife,"weibull",SEs = T)
m2=fc_fit(taglife,"weibull3",SEs = T)

# 
fmods_all=fc_fit(taglife,names(fc_mod_ls)[1:7],SEs = T)

# 
# fmods_all=suppressWarnings(fc_fit(taglife,"all",SEs = T))
# fmods_all=suppressWarnings(fc_fit(taglife,"all",SEs = F))
fmods_all
